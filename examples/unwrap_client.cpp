...
auto client(std::string_view ip, uint16_t port, int num_connections) -> Task<void> {
    ...
    if (auto stream_ret = co_await io::connect(fd,
        ...
        char             r_buf[1024];
        while (true) {
            if (auto ret = co_await io::send(fd, w_buf.data(), w_buf.size(), MSG_NOSIGNAL)) {
        ...
        LOG_ERROR("connect failed, {}", stream_ret.error().message());
    }
